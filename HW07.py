...
def litCricFriend(wordList, text):
    ...
    textList = []
    for i in text:
        textList.append(i.strip(string.punctuation))
    textCount = 0
    for i in textList:
        for word in wordList:
            if word in i:
                textCount += 1
    return textCount / len(textList)
...
print(litCricFriend(['an', 'a'], theBells))
print(litCricFriend(['an', 'a'], cantoXII))
...
The results show that the ratio when using 'a' and 'an' are
very close, both with a ratio of about 0.3
The ratio for using 'the' is a little different, with Poe
having one of .17 and Neruda having a ratio of .07
'''
